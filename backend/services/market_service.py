...
def get_historical_price(symbol: str, limit: int = 50, interval: str = "1h") -> list:
    ...
    for attempt in range(3):  # retry up to 3 times for Render cold starts
        try:
            response = requests.get(url, timeout=15)
            if response.status_code == 200:
                data = response.json()
            ...
            print(f"Bybit kline response error for {symbol} (attempt {attempt+1}): {response.text}")
        except Exception as e:
            print(f"Error fetching historical prices from Bybit for {symbol} (attempt {attempt+1}): {e}")
    return []
